...
class Extractor(nn.Module):
    def __init__(self):
        ...
        # 26 x 19
        self.conv4 = create_layer(["CONV",ch2, ch2, c_k_last_size, c_st, pad])
        # 15 x 10
    ...
    def forward(self, x):
        ...
        # print("after max_pool1 flatten", x.shape)
        x = x.permute(1,0,2)
        # print("after max_pool1 permute", x.shape)
        x = torch.unsqueeze(x, 0)
        # print("after max_pool1 unsqueeze", x.shape)
        x1 = self.conv5(F.dropout(x,p = 0.5, training=self.training))
        # print("after conv5", x1.shape)
        x2 = self.conv6(F.dropout(x,p = 0.5, training=self.training))
