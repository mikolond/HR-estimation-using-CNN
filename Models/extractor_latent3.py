...
class Extractor(nn.Module):
    def __init__(self):
        ...
        self.conv1 = create_layer(["CONV", in_ch, ch, c_k_size, c_st, pad,"MP" ,m_k_size, (2,2), "BN", ch,"ELU", alpha_elu])
        # 82 x 55
        ...
        # 26 x 19
        self.conv4 = create_layer(["CONV",ch, ch, (12,8), c_st, pad,"MP",(12,8), m_st, "BN", ch,"ELU", alpha_elu])
        # 4 x 5
        # make 88 x 1 vector
        ...
        self.conv_trans1 = create_layer(["CONV_trans",ch, ch, (3,3), (2,1), (0,0), (1,0), "ELU", alpha_elu])
        self.conv14 = create_layer(["CONV",ch, ch, (3,3), (1,2), (1,0), "ELU", alpha_elu])

        self.conv_trans2 = create_layer(["CONV_trans",ch, ch, (3,3), (2,1), (0,0), (1,0), "ELU", alpha_elu])
        self.conv15 = create_layer(["CONV",ch, ch, (3,3), (1,2), (1,0), "ELU", alpha_elu])

        self.conv_trans3 = create_layer(["CONV_trans",ch, ch, (3,3), (2,1), (0,0), (1,0), "ELU", alpha_elu])
        self.conv16 = create_layer(["CONV",ch, ch, (3,3), (1,2), (1,0),"ELU", alpha_elu])

        # self.maxpool4 = nn.MaxPool2d(kernel_size=(1,5), stride=(1,1), padding=(0,0))
        self.conv17 = create_layer(["CONV",ch, ch, (3,7), (1,1), (1,0), "ELU", alpha_elu])
    ...
    def forward(self, x):
        ...
        # x = x / 255 * 2 - 1
        x = self.bn_input(x)
        x = self.conv1(F.dropout2d(x,p = 0.01, training=self.training))
        x = self.conv2(F.dropout(x,p = 0.01, training=self.training))
        x = self.conv3(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv3:",x.shape)
        x = self.conv4(F.dropout2d(x,p = 0.01, training=self.training))
        # print("after conv4:",x.shape)
        x = torch.flatten(x, start_dim=2)
        x = x.permute(1,0,2)
        x = torch.unsqueeze(x, 0)
        # print("after flatten and dimension unsqueeze:",x.shape)
        x1 = self.conv5(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv5:",x1.shape)
        x2 = self.conv6(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv6:",x2.shape)
        x3 = self.conv7(F.dropout(x,p = 0.01, training=self.training))
        ...
        x1 = self.conv8(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv8:",x1.shape)
        x2 = self.conv9(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv9:",x2.shape)
        x3 = self.conv10(F.dropout(x,p = 0.01, training=self.training))
        ...
        x1 = self.conv11(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv11:",x1.shape)
        x2 = self.conv12(F.dropout(x,p = 0.01, training=self.training))
        # print("after conv12:",x2.shape)
        x3 = self.conv13(F.dropout(x,p = 0.01, training=self.training))
        ...
        x = self.conv_trans2(F.dropout(x,p = 0.2, training=self.training))
        # print("after conv_trans2:",x.shape)
        x = self.conv15(F.dropout(x,p = 0.2, training=self.training))
        # print("after conv15:",x.shape)

        x = self.conv_trans3(F.dropout(x,p = 0.2, training=self.training))
        # print("after conv_trans3:",x.shape)
        x = self.conv16(F.dropout(x,p = 0.3, training=self.training))
