...
class Extractor(nn.Module):
    def __init__(self):
        ...
        # batch_size x 12
        self.maxpool1 = nn.MaxPool2d(kernel_size=(1,4), stride=(1,2), padding=(0,0))
        ...
        self.conv_cat1 = create_layer(["CONV",ch, ch, (3,3), (1,1), (1,1),"MP",(3,3),(2,2), "ELU", alpha_elu])
        self.maxpool2 = nn.MaxPool2d(kernel_size=(1,5), stride=(1,1), padding=(0,0))
        ...
        self.conv_cat2 = create_layer(["CONV",ch, ch, (3,3), (1,1), (1,1),"MP",(3,3),(2,2), "ELU", alpha_elu])
        self.maxpool3 = nn.MaxPool2d(kernel_size=(1,14), stride=(1,1), padding=(0,0))
        ...
        self.conv_cat3 = create_layer(["CONV",ch, ch, (3,3), (1,1), (1,1),"MP",(3,3),(2,2), "ELU", alpha_elu])
        ...
        self.conv_trans2 = create_layer(["CONV_trans",2*ch, ch, (4,1), (2,1), (0,0), (0,0), "ELU", alpha_elu])
        self.conv15 = create_layer(["CONV",2*ch, ch, (3,6), (1,2), (1,0), "ELU", alpha_elu])

        self.conv_trans3 = create_layer(["CONV_trans",ch, ch, (4,1), (2,1), (0,0), (0,0), "ELU", alpha_elu])
        self.conv16 = create_layer(["CONV",2*ch, ch, (3,5), (1,2), (1,0),"ELU", alpha_elu])
    ...
    def forward(self, x):
        ...
        # x = x / 255 * 2 - 1
        x = self.bn_input(x)
        x = self.conv1(F.dropout2d(x,p = 0.2, training=self.training))
        ...
        x_skip1 = self.maxpool1(x)
        # print("after maxpool1:",x_skip1.shape)
        x1 = self.conv5(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv5:",x1.shape)
        x2 = self.conv6(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv6:",x2.shape)
        x3 = self.conv7(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv7:",x3.shape)
        x = torch.cat((x1,x2,x3),3)
        # print("after conv5,6,7 cat:",x.shape)
        x = self.conv_cat1(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv_cat1:",x.shape)
        x_skip2 = self.maxpool2(x)
        # print("after maxpool2:",x_skip2.shape)

        x1 = self.conv8(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv8:",x1.shape)
        x2 = self.conv9(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv9:",x2.shape)
        x3 = self.conv10(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv10:",x3.shape)
        x = torch.cat((x1,x2,x3),3)
        # print("after conv8,9,10 cat:",x.shape)
        # x = self.maxpool2(x)
        x = self.conv_cat2(F.dropout(x,p = 0.1, training=self.training))
        # # print("after maxpool2:",x.shape)
        # print("after conv_cat2:",x.shape)
        x_skip3 = self.maxpool3(x)
        # print("after maxpool3:",x_skip3.shape)

        x1 = self.conv11(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv11:",x1.shape)
        x2 = self.conv12(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv12:",x2.shape)
        x3 = self.conv13(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv13:",x3.shape)
        x = torch.cat((x1,x2,x3),3)
        # print("after conv11,12,13 cat:",x.shape)
        # x = self.maxpool3(x)
        x = self.conv_cat3(F.dropout(x,p = 0.1, training=self.training))
        # # print("after maxpool3:",x.shape)
        # print("after conv_cat3:",x.shape)

        x = self.conv_trans1(F.dropout(x,p = 0.1, training=self.training))
        # print("after conv_trans1:",x.shape)
        x = self.conv14(F.dropout(x,p = 0.1, training=self.training))
