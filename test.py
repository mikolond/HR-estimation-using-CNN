...
CONFIG_PATH = os.path.join("config_files", "cross_val", "ecg", "new", "config_eval_split3.yaml")
...
if __name__ == "__main__":
    import yaml
    import csv
    # load the predicted and truth values
    path_test = os.path.join("output", "ecg_model5_exp1","predictions_validation.txt")
    ...
    print("predicted", predicted)
    plot_pearson(ground_truth, predicted, os.path.join("output", "ecg_model5_exp1"), "validation")
    min_hr = 40
    max__hr_data = max(max(ground_truth), max(predicted))
    print("max__hr_data", max__hr_data)
    max_hr = max(160, max__hr_data+10)//10*10
    print("max_hr", max_hr)
    errors_per_class = get_per_class_error(ground_truth, predicted,min_hr=min_hr, max_hr=max_hr)
    plot_per_class_error(errors_per_class, os.path.join("output", "ecg_model5_exp1"), "validation", min_hr=min_hr, max_hr=max_hr)
